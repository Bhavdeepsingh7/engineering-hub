...
const ACCEPTED = [".pdf", ".txt", ".md", ".pptx"];
...
export function UploadZone({ onUpload }) {
  ...
  const validate = (files) => {
    ...
    if (invalid.length) setError(`Unsupported: ${invalid.join(", ")}. Use PDF, TXT, MD, or PPTX.`);
    else setError("");
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" multiple accept=".pdf,.txt,.md,.pptx" className="hidden" onChange={(e) => validate(e.target.files)} />
      ...
      <p className="text-xs text-surface-400 mt-1">Supports PDF, TXT, Markdown, and PPTX files</p>
      {error && (
